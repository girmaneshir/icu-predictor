...
class LOSPredictor:
    def __init__(self, model_path='models/los/best_model.joblib', 
                 preprocessor_path='models/los/preprocessor.joblib'):
        try:
            self.model = joblib.load(model_path)
            self.preprocessor = joblib.load(preprocessor_path)
        except Exception as e:
            st.error(f"🚨 LOS model loading error: {str(e)}")
            self.model = None
            self.preprocessor = None
    
    def predict(self, patient_data):
        """Predict LOS for a new patient record"""
...
class MortalityPredictor:
    def __init__(self, model_path='models/mortality/best_model.joblib', 
                 preprocessor_path='models/mortality/preprocessor.joblib'):
        try:
            self.model = joblib.load(model_path)
            self.preprocessor = joblib.load(preprocessor_path)
        except Exception as e:
            st.error(f"🚨 Mortality model loading error: {str(e)}")
            self.model = None
            self.preprocessor = None
    
    def predict(self, patient_data):
        """Predict mortality risk for a new patient record"""
        if self.model is None or self.preprocessor is None:
            return None, None
        ...
        try:
            ...
            # Make prediction
            mortality_prob = self.model.predict_proba(processed_data)[0][1]  # Probability of death
            prediction = 1 if mortality_prob > 0.5 else 0  # Binary prediction
...
# Configure page
st.set_page_config(
    page_title="ICU Clinical Predictor Suite", 
    layout="wide", 
    page_icon="🏥",
...
if tool == "Combined View":
    ...
    if st.button("Generate Comprehensive Risk Assessment"):
        ...
        if los_prediction is not None:
            ...
            patient_data_with_los['LOS'] = los_prediction
            
            # Predict mortality
            mortality_pred, mortality_prob = mortality_predictor.predict(patient_data_with_los)
            
            if mortality_pred is not None and mortality_prob is not None:
